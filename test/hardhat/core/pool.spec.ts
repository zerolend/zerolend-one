...
describe('Pool', () => {
  ...
  describe('Supply / Withdraw functions', () => {
    ...
    it('try to supply into a pool', async () => {
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
    });

    it('try to withdraw after a supply into a pool', async () => {
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
      await pool['withdraw(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
    });

    it('should give right balances for supploed positions', async () => {
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('2'), 1);
    ...
    it('should revert if withdraw after another index', async () => {
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
      const t = pool['withdraw(address,uint256,uint256)'](tokenA.target, eth('1'), 1);
      await expect(t).to.revertedWith('Insufficient Balance!');
    });

    it('should revert if withdraw more than supplied', async () => {
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('1'), 0);
      const t = pool['withdraw(address,uint256,uint256)'](tokenA.target, eth('10'), 0);
      await expect(t).to.revertedWith('Insufficient Balance!');
    });
  });
  ...
  describe('Borrow / Repay functions', () => {
    beforeEach(async () => {
      ...
      await pool['supply(address,uint256,uint256)'](tokenA.target, eth('5'), 0);
      await pool['supply(address,uint256,uint256)'](tokenB.target, eth('5'), 0);
    });

    it('try to borrow from a pool', async () => {
      expect(await tokenB.balanceOf(deployer.address)).eq(eth('5'));
      await pool['borrow(address,uint256,uint256)'](tokenB.target, eth('1'), 0);
    ...
    it('try to partial repay whatever was borrowed', async () => {
      ...
      await pool['borrow(address,uint256,uint256)'](tokenB.target, eth('1'), 0);
      expect(await tokenB.balanceOf(deployer.address)).eq(eth('6'));
      await pool['repay(address,uint256,uint256)'](tokenB.target, eth('0.1'), 0);
