...
abstract contract Pool is Initializable, IPool {
    ...
    mapping(bytes32 position => DataTypes.UserConfigurationMap config)
        internal _usersConfig;
    ...
    mapping(address reserve => DataTypes.ReserveSupplies totalSupply)
        internal _totalSupplies;
    ...
    mapping(address reserve => IAggregatorInterface oracle)
        internal _assetsSources;
     ...
     */
    function initialize(
        IPool.InitParams memory params
        ...
        for (uint i = 0; i < params.assets.length; i++) {
            ...
            PoolLogic.executeInitReserve(
                ...
                DataTypes.InitReserveParams({
                    ...
                    interestRateStrategyAddress: params.rateStrategyAddresses[
                        i
                    ],
    ...
    function withdraw(
        ...
        require(
            amount <= _balances[asset][positionId].scaledSupplyBalance,
    ...
    function _setReserveConfiguration(
        ...
        require(
            config.getLtv() <= config.getLiquidationThreshold(),
        ...
        if (config.getLiquidationThreshold() != 0) {
            ...
            require(
                config.getLiquidationThreshold().percentMul(
    ...
    function setReserveConfiguration(
        ...
        require(
            msg.sender == address(factory.configurator()),
            "only configurator"
        );
        _setReserveConfiguration(
            asset,
