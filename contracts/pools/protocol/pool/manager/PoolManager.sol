...
contract PoolManager is TimelockedActions {
  ...
  function addPoolAdmin(address pool, address admin) external override {
    grantRole(getRoleFromPool(pool, POOL_ADMIN_ROLE), admin);
  }

  function removePoolAdmin(address pool, address admin) external override {
    revokeRole(getRoleFromPool(pool, POOL_ADMIN_ROLE), admin);
  }

  function isPoolAdmin(address pool, address admin) external view override returns (bool) {
    return hasRole(getRoleFromPool(pool, POOL_ADMIN_ROLE), admin);
  }

  function addEmergencyAdmin(address pool, address admin) external override {
    grantRole(getRoleFromPool(pool, EMERGENCY_ADMIN_ROLE), admin);
  }

  function removeEmergencyAdmin(address pool, address admin) external override {
    revokeRole(getRoleFromPool(pool, EMERGENCY_ADMIN_ROLE), admin);
  }

  function isEmergencyAdmin(address pool, address admin) external view override returns (bool) {
    return hasRole(getRoleFromPool(pool, EMERGENCY_ADMIN_ROLE), admin);
  }

  function addRiskAdmin(address pool, address admin) external override {
    grantRole(getRoleFromPool(pool, RISK_ADMIN_ROLE), admin);
  }

  function removeRiskAdmin(address pool, address admin) external override {
    revokeRole(getRoleFromPool(pool, RISK_ADMIN_ROLE), admin);
  }

  function isRiskAdmin(address pool, address admin) external view override returns (bool) {
